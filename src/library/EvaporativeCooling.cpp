...
EvaporativeCooling::EvaporativeCooling(Dataset* ds, po::variables_map& vm,
  ...
  if(paramsMap.count("ec-iter-remove-percent")) {
    ...
    numToRemovePerIteration = (unsigned int) (((double) iterPercentToRemove / 100.0) *
  ...
  cout << Timestamp() << maxThreads << " OpenMP processors available to EC" << endl;
...
//      return false;
  ...
  cout << Timestamp() << "EC algorithm ran for " << iteration
          << " iterations" << endl;
...
bool EvaporativeCooling::PrintAllScoresTabular() {
  ...
  if(rjScores.size() != rfScores.size()) {
    cerr << "ERROR: Random Jungle and Relief-F scores lists are not the same size"
            << endl;
    return false;
  }
